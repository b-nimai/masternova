...
> switch at the **1.5–2 YoE SDE** level. Replaces *Loom Lite AI*. Built from an existing
...
| Async pipeline | transcode + AI | same pipeline, *plus* it now gates paid content |
...
"how I'd split this at 10x" section (§10) proves you *chose* rather than defaulted.
...
- **Transactional outbox**: order state change + outbox rows committed in ONE transaction;
  a relay worker publishes → enroll, invoice, email. Gives you exactly-once *effects*
...
- Discuss: at-least-once beats, monotonic `maxPositionSeconds`, and why last-write-wins is
  acceptable *here* (and where it wouldn't be).
...
Naming the *breaking points* is what separates a senior answer from a buzzword answer.
...
| 4 | Sep 14–20 | **Entitlement engine** + playback tokens + CloudFront-style signed access; progress heartbeats w/ Redis write-back | Non-enrolled user blocked at API *and* at the CDN path |
...
Being able to say *"I scoped that out on purpose, and here's the design I'd build"* is a stronger
