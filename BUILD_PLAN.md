...
| [1A — Backend: core spine](#6-phase-1a--backend-core-spine) | 11     | 5      | 180 h     | ~44 h | 🔨     |
...
| **Specification ships two representations, not an ORM-neutral AST** (1.4) | `toWhere()` returns a `Prisma.CourseWhereInput`, so a specification is not ORM-agnostic. The neutral alternative is a predicate AST plus a translator — a query builder written to dodge a dependency ADR-0001 already committed to. The drift risk is real and is guarded by an integration test that runs every leaf both ways and compares ids.                                                                                                                                                                                                                                                                                                                                                       |
| **Keyset clause ships as a disjunction, not a row comparison** (1.4)      | Measured: the `OR` form is 0.964 ms at page 50 against 1.133 ms for plain `OFFSET` — a disjunction cannot be an index _start_ condition. The row-comparison form is 0.121 ms and Prisma cannot emit it; its own `cursor` emits an OR-chain over correlated subqueries, which is worse. Composability kept, number recorded, breaking point named in ADR-0015.                                                                                                                                                                                                                                                                                                                                            |
| **`(status, priceMinor)` measured and NOT shipped** (1.4)                 | A 16.9%-selective facet is not an index, and the planner never chose it while the main composite existed. Kept in `docs/db/indexes.md` §7 with its numbers — a recorded loser is more credible than six indexes that all "helped". Same for the partial index Prisma cannot declare.                                                                                                                                                                                                                                                                                                                                                                                                                     |
| **Two production bugs found by running it, not by review** (1.4)          | `JwtAuthGuard` returned early on `@Public()` routes before parsing the token, so the public catalog saw every signed-in instructor as a stranger. And the authoring endpoints returned raw Prisma rows, so a replayed `Idempotency-Key` handed back a `Decimal` as `{d,e,s}` where the original sent `0`. Both fixed here.                                                                                                                                                                                                                                                                                                                                                                               |
| **Builder dropped from 1.5**                                              | The task named it for "stepwise assembly, validated before legal". The publish gate is a list of ten independent named requirements, not a construction sequence — and the property that makes it valuable is that the _same list_ produces both the 422 and the wizard's per-step checklist, which a builder would obscure. It has no force here, and §3 says an abstraction without one gets deleted. The real Builder is the ffmpeg HLS command (1.7).                                                                                                                                                                                                                                                |
| **State ships as a data edge-list, not a class per state** (1.5)          | Textbook State is a class per state with a method per event: four states × five events, seventeen of them `throw`. The interesting content here is the edge list, and burying it in method bodies makes "what can happen next?" — the one question the wizard asks to enable its buttons — the hardest thing to answer. The property that matters is preserved: no path in the file produces an edge the source state did not declare. Argued in the file and in `../LLD/6. Behavioural Design Patterns/3. State Pattern.md`.                                                                                                                                                                            |
| **`DRAFT → PUBLISHED` removed; approval is ADMIN-only** (1.5)             | Task 1.4 let an instructor publish directly. Kept, `IN_REVIEW` would be a state nothing meaningfully enters and the enum would document a workflow that does not exist. Instructor submits, reviewer approves. A unit test asserts the shortcut's absence specifically, and three tests in the 1.4 suite were rewritten to walk the real path.                                                                                                                                                                                                                                                                                                                                                           |
| **"all media READY" narrowed to "has an asset"** (1.5)                    | The gate as specified needs to know whether transcoding finished, and `media` (1.6) does not exist. Inventing its readiness contract from catalog's side would mean changing it in two months. The requirement's predicate widens when 1.6 lands and nothing else in `publish-gate.ts` moves — the seam is named in the file and in the LLD's §11.                                                                                                                                                                                                                                                                                                                                                       |
| **`Course.priceSetAt` added** (1.5, not in the plan)                      | "Price set" is unimplementable against `priceMinor` alone: `0` is both "this course is free" and "nobody has priced it yet". Without the column the gate either bans free courses or waves through a paid one an instructor forgot to price — and the second costs real money. One nullable timestamp, stamped once.                                                                                                                                                                                                                                                                                                                                                                                     |
| **`details` added to the error envelope** (1.5)                           | The publish gate rejects with a _list_ of coded problems, and flattening that into one sentence makes the client parse prose to decide which wizard step to open. `message` stays the human sentence; `details` carries the codes. Recorded as a rule in the new `docs/api/conventions.md`, which task 1.11 finishes.                                                                                                                                                                                                                                                                                                                                                                                    |
| **`docs/api/conventions.md` started early** (1.5)                         | Task 1.11 owns it, but three rules landed here — the `details` field, the `expectedVersion` contract, and where `Idempotency-Key` is used _instead of_ a version. §7 is explicit that a doc written later is written from memory and therefore wrong, so the file exists now marked partial rather than being reconstructed in week eight.                                                                                                                                                                                                                                                                                                                                                               |
| **Three bugs found by `code-review`, all in older tasks** (1.5)           | Fixed here because this is the commit that noticed them, each with a regression test. **(a)** `?minPrice=` with no `maxPrice` passed `Number.MAX_SAFE_INTEGER` into an INT4 column — a 500 before the query reached Postgres (1.4). **(b)** The idempotency `requestHash` covered only the body, and both idempotent routes send **no body**, so one key reused across two courses replayed the first course's response with a 200 (1.1). Now hashes the concrete target too. **(c)** `revokeOnReuse` published unconditionally, so replaying one stolen refresh token N times sent N security emails — an unauthenticated mail-bomb (1.2). Now guarded on the revoke count, as its sibling already was. |
| **Transitions apply conditionally on the source state** (1.5)             | The state machine validated the edge against a row read _outside_ the transaction and then wrote unconditionally, so an archive and a publish — each legal from what its caller saw — could both land and leave a course PUBLISHED after being archived. "ARCHIVED is terminal" was a comment, not a guarantee. `setStatus` now takes `expectedFrom` and 409s on a miss. A transition also bumps `version`, which closes the matching check-then-act window on the archived-course guard.                                                                                                                                                                                                                |
...
- [x] **Two concurrent wizard saves → one wins, the other gets a 409** (1.5 — `apps/api/test/catalog-authoring.int-spec.ts`; also 10 concurrent commands → exactly 1 success)
- [ ] Policy chain unit tests — **no database**
- [x] **Every curriculum command round-trips through its own inverse** (1.5 — `curriculum-commands.spec.ts`, no database)
...
| 0015 | Keyset cursors over LIMIT/OFFSET pagination           | 1.4   | ✅     |
| 0016 | Optimistic concurrency for authoring writes           | 1.5   | ✅     |
...
`LLD/6. Behavioural Design Patterns/` now exists, with Observer, State and Command written
against real Masternova code. **Strategy is the remaining P0** and arrives with the
entitlement policies (task 1.8).
